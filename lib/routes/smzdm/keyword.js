...
module.exports = async (ctx) => {
    ...
    const response = await got({
        method: 'get',
        url: `http://search.smzdm.com/?c=faxian&s=${encodeURI(keyword)}&order=time&v=b`,
        headers: {
            Referer: `http://search.smzdm.com/?c=faxian&s=${encodeURI(keyword)}&order=time&v=b`,
        },
    });
    ...
    ctx.state.data = {
        allowEmpty: true,
        title: `${keyword} - 什么值得买`,
        link: `http://search.smzdm.com/?c=faxian&s=${encodeURI(keyword)}&order=time&v=b`,
        item:
            list &&
            list
                .map((index, item) => {
